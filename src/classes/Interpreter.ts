...
class Interpreter {
    ...
    constructor(
        ...
        if (chanceCallback === undefined) {
            this.chanceCallback = (d: number): boolean =>
                d == 1 || Math.random() < d;
        } else {
    ...
    runScene(sceneName: string): void {
        if (
            !this.binary.translationLoaded &&
            this.binary.translationTable.length == 0
        ) {
            console.log(
                "[WARNING]: Currently no translations have been loaded! The program will crash when trying to run dialogue!"
            );
        }
        ...
        for (
            let i: number = 1, flagIndex = 0;
            i < bytecodeIndexes.length;
            flagIndex++
        ) {
    ...
    chooseChoice(idx: number): void {
        if (idx >= this.#choices.length) {
            throw new Error(
                `Choice at index ${idx} is outside of the range of choices.`
            );
        }
    ...
    update(): void {
        ...
        let opcode: Opcode =
            this.binary.instructions[this.#instructionPointer++];
        let arg1: number = 0;
        let arg2: number = 0;
        ...
        switch (opcode) {
            ...
            case Opcode.ChooseAddTruthy:
                arg1 = this.binary.instructions.readInt32LE(
            ...
            case Opcode.CallExternal:
                ...
                arg2 = this.binary.instructions.readInt32LE(
                    this.#instructionPointer
                );
                this.#instructionPointer += 4;
                break;
            ...
            case Opcode.PushDouble:
                arg1 = this.binary.instructions.readDoubleLE(
                    this.#instructionPointer
                );
        ...
        switch (opcode) {
            ...
            case Opcode.PushInterpolatedString:
                this.#stack.push(
                    this.interpolate(this.binary.translationTable[arg1], arg2)
                );
                break;
            ...
            case Opcode.PushBinaryInterpolatedString:
                this.#stack.push(
                    this.interpolate(this.binary.stringTable[arg1], arg2)
                );
                break;
            ...
            case Opcode.PushVarGlobal:
                this.#stack.push(
                    this.globalVariableStore[this.binary.stringTable[arg1]]
                );
                break;
            ...
            case Opcode.BitLeftShift: {
                ...
                let val: Value = this.#stack.pop();
                if (typeof shift === "number" && typeof val === "number")
                    this.#stack.push(val << shift);
                break;
            }
            ...
            case Opcode.Call: {
                ...
                this.#stack = [];
                this.#localVarStore = new LocalVariableStore(this);
                let bytecodeIndexes: number[] =
                    this.binary.functions[arg1].instructionIndices;
            ...
            case Opcode.ChoiceBegin:
                if (this.inChoice) {
                    throw new InterpreterRuntimeException(
                        "Choice begins while another choice is being processed!"
                    );
                }
            ...
            case Opcode.ChoiceAdd: {
                if (!this.inChoice) {
                    throw new InterpreterRuntimeException(
                        "Attempted to add a choice when no choice is being processed!"
                    );
                }
            ...
            case Opcode.ChoiceSelect: {
                if (!this.inChoice) {
                    throw new InterpreterRuntimeException(
                        "Attempted to wait for user choice when no choice is being processed!"
                    );
                }

                if (this.#choices.length == 0) {
                    throw new InterpreterRuntimeException(
                        "Attempted to wait for user choice when there's no choices to choose!"
                    );
                }
                ...
                if (typeof chance === "number") {
                    if (opcode != Opcode.ChooseAddTruthy || this.#stack.pop()) {
                        this.#chooseOptions.push({
                            value: chance,
                            pointer: this.#instructionPointer + arg1
                        });
                    }
                }
                break;
            ...
            case Opcode.ChooseSelect: {
                let selection: number = this.weightedChanceCallback(
                    this.#chooseOptions.map((t) => t.value)
                );

                if (
                    selection == -1 ||
                    selection >= this.#chooseOptions.length
                ) {
                    throw new Error(
                        `Selection returned by WeightedChanceCallback was out of bounds. Selection: ${selection}`
                    );
                }

                this.#instructionPointer =
                    this.#chooseOptions[selection].pointer;
                this.#chooseOptions = [];
                break;
            }
    ...
    getDefinition(def: string | Definition): string {
        ...
        let value: string =
            (strRef ^ (1 << 31)) == 0 ? this.binary.stringTable[strRef & 0x7fffffff] : this.binary.translationTable[strRef];
    ...
    lookupScene(sceneName: string): number {
        ...
        const scene: number = this.binary.scenes.findIndex(
            (s) => s.symbol === id
        );
        if (scene === -1) {
            throw new InterpreterRuntimeException("Scene could not be found!");
    ...
    lookupFunction(funcName: string): number {
        ...
        const func: number = this.binary.functions.findIndex(
            (s) => s.symbol === id
        );
        if (func === -1) {
            throw new InterpreterRuntimeException(
    ...
    lookupDefinition(defName: string): Definition {
        ...
        const def: Definition | undefined = this.binary.definitions.find(
            (s) => s.symbol === id
        );
        if (def === undefined) {
            throw new InterpreterRuntimeException(
